...
async function stickerSender(sock, msg, sizeOption) {
    ...
    if (msg.message.conversation) {
        ...
        if (msg.message.extendedTextMessage.contextInfo.quotedMessage.videoMessage) {
            if (msg.message.extendedTextMessage.contextInfo.quotedMessage.videoMessage.seconds > vlen) {
                let sentMsg = await sock.sendMessage(id, { text: '🎥 Trimmed video to maximum ' + vlen + ' seconds 🕛' }, { quoted: msg });
            }
            isVideo = true;
