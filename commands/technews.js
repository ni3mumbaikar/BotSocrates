        ...
        if (subscriber == undefined) {
            ...
            if (msg.pushName && id) {
                ...
                if (id.endsWith("@g.us")) {
                    ...
                    let sentMsg = await sock.sendMessage(id, {
                        text: "✅ Technews subscription activated for 3 months !" + "\n"
                            + "You'll receive daily technews twice @7:00AM and 7:00PM" + "\n\n" +
                            "Note : Subscription is valid till _" + currentmomment.format("ddd Do MMM YYYY") + "_"
                    }, { quoted: msg });
                } else {
                    let sentMsg = await sock.sendMessage(id, {
            ...
            if (subscriber.valid == false) {
                ...
                if (result.matchedCount == 1) {
                    if (moment().isBefore(moment(subscriber.valid_till))) {
                        ...
                        let sentMsg = await sock.sendMessage(id, {
                            text: "✅ Technews subscription is active !" + "\n"
                                + "You'll receive daily technews twice @7:00AM and 7:00PM" + "\n\n" +
                                "Note : Subscription is valid till _" + valid_till.format("ddd Do MMM YYYY") + "_"
                        }, { quoted: msg });
                    } else {
                        let currentmomment = moment().add(90, 'days');
                        let sentMsg = await sock.sendMessage(id, {
                            text: "✅ Technews subscription renewed for 3 months !" + "\n"
                                + "You'll receive daily technews twice @7:00AM and 7:00PM" + "\n\n" +
                                "Note : Subscription is valid till _" + currentmomment.format("ddd Do MMM YYYY") + "_"
                        }, { quoted: msg });
                    }
